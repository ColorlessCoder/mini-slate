...
export default ({ blocks }, editor) => {
  ...
  if(editor.value.document.getAncestors(listItem.key).toArray().filter(node => node.type === list.type).length >= blocks.maximumDepth) {
    return;
  }
  ...
  // list if it exists.
  const existingList = previousListItem.nodes.filter(node => node.type === list.type).get(0);
